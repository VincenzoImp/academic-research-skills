...
1. Split every review into atomic concerns.
2. Classify each concern: misunderstanding, valid limitation, missing evidence,
   writing issue, incorrect claim, scope mismatch, or unfixable weakness.
3. Decide action: concede, clarify, add evidence, reframe, defend, or defer.
4. Draft response with direct answer first, then evidence, then promised change.
5. Avoid new claims that cannot be added to the paper or artifact.
6. Save concern mapping in `reports/rebuttal/`.
7. Update manuscript, `docs/methodology/threats-to-validity.md`, and wiki claim pages.
